...
-- unique number of departures connections
	  ...
	  --FROM prep_flights  --- to check in Dbeaver
		GROUP BY origin
),
arrivals AS (
	--airport as destinations
	SELECT dest, 
...
total_stats AS (
		SELECT origin AS airport_code,
				...
				d.dep_cancelled + arr_cancelled AS total_cancelled,
				d.dep_diverted + a.arr_cancelled AS total_diverted,
...
--JOIN prep_airports ap   --- to check in Dbeaver
